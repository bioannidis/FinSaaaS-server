    ...
    public class ValueEstimator {
        ...
        public static float valueEstimation(CostProfile costprofus, MapfromXmltrack map, double near_area) {
            float value = 0;
        ...
        public static float valueEstimation(CostProfile costprofus, MapfromXmltrack map) {
            float value = 0;
            GeoLocation geo = GeoConverter(costprofus);
            Point loc;
            SortedMap.Entry pair;
        ...
        public static int getValueofMap(MapfromXmltrack map){
            Point loc;
            int val=0;
            SortedMap.Entry pair;
               double minDist = 10000000, currDist;
            SortedMap<Double, Point> sub_map = map.nav_map;
            Iterator it = sub_map.entrySet().iterator();
            while (it.hasNext()) {
                pair = (SortedMap.Entry) it.next();
                loc = (Point) pair.getValue();
                val=val+loc.timesSensed;
            }
            return val;
        }
        /*float value=0;
